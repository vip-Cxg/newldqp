let calc = require('../Hulib/calc19');//require("../Hulib/calc19");
...
const SHARE_URL = "https://www.p64sd.cn/index_00.html";
...
cc.Class({
    ...
    onLoad() {
        ...
        this.schedule(this.gameVoiceSchedule, 0.1);
        // },cc.sys.isNative ? 0 : 2000);
        //cc.log(this.lblTime);
        //回复服务器表示客户端初始化完毕 
        //TODO 只有JOINDONE
        // if (db.connectInfo && db.connectInfo.reconnect) {
    ...
    initChatContent() {
        this.node.on('chatAlready', () => {
            ...
            let data = {
                ...
                url: 'ChatImg/Game11',
                aniPos: [
                    cc.v2(139 / 2 - windowNode.width / 2 + GameConfig.FitScreen, -100 + 46),
                    cc.v2(-139 / 2 + windowNode.width / 2 - GameConfig.FitScreen, 50 + 46),
                ],
                msgPos: [
                    cc.v2(139 / 2 - windowNode.width / 2 + GameConfig.FitScreen + 50, -100 + 83),
                    cc.v2(-139 / 2 + windowNode.width / 2 - GameConfig.FitScreen - 50, 50 + 50),
                ],
                facePos: [
                    cc.v2(139 / 2 - windowNode.width / 2 + GameConfig.FitScreen + 50, -100),
                    cc.v2(-139 / 2 + windowNode.width / 2 - GameConfig.FitScreen + 50, 50),
                ],
    ...
    /**初始化房间信息显示 */
    initTableMsg(data) {
        this.lblBase.string = "底分: ¥" + data.options.base;//utils.formatGold(data.options.base);
    ...
    gameInit(data) {
        ...
        this.nodeBird.parent.active = false;
    ...
    //         'an': 1
    //     };
    //     this.actionKuang[idx].active = true;
    //     if (this.actionKuang[idx])
    //         this.actionKuang[idx].opacity = 0;
    //     this.actionKuang[idx].stopAllActions();
    //     let sprite = this.actionKuang[idx].getChildByName('gang').getComponent(cc.Sprite);
    //     sprite.spriteFrame = this.sprkuang[spr[type]];
    //     this.actionKuang[idx].runAction(cc.sequence(
    //         cc.callFunc(() => {
    //             this.actionKuang[idx].scale = 0;
    //         }),
    //         cc.spawn(
    //             cc.fadeIn(0.2),
    //             cc.scaleTo(0.3, 0.7)
    ...
    roundSummary(data) {
        ...
        node.getComponent('ModuleRoundSummary19').init(data);
        node.active = true;
        data.players.forEach((p, i) => {
            ...
            if (!utils.isNullOrEmpty(p.total))
                TableInfo.players[p.idx].total = p.total;
    ...
    sendReady(e) {
        ...
        // connector.gameMessage(TableInfo.status == GameConfig.GameStatus.CHAPTER ? ROUTE.CS_CHAPTER : ROUTE.CS_GAME_READY, {});
        this.btnContinue.active = false;
    },
    ...
    reconnect(data) {
        ...
        if (data.quickFinish) {
            this.nodeTx.getComponent("ModuleTouxiang19").txInit(data.quickFinish);
        }
        if (data.status == GameConfig.GameStatus.START || data.status == GameConfig.GameStatus.PLAY || data.status == GameConfig.GameStatus.QUEST) {
            ...
            //this.lblDeck.string = "" + data.decks;
            // if (data.currentPlayer != null && data.currentCard == null && data.status != GameConfig.GameStatus.QUEST) {

            if (data.currentPlayer != null && data.currentCard == null) {
                let msg = { idx: data.currentPlayer, clock: data.clock, serialID: data.serialID };
                this.playCard(msg);
                TableInfo.currentPlayer = data.currentPlayer;
            }
            //自己的ground区 补花区
            ...
            data.players.forEach(player => {
                if (player) {
                    ...
                    this.players[idx].active(data.currentPlayer);
    ...
    gameVoiceSchedule() {
        ...
        if (connector._queueChatMsg.length <= 0)
            return;
        let msg = connector._queueChatMsg.shift();
        this.playSpeech(msg);
    },

    playSpeech: function (data) {
        ...
        this.timeCell = Math.ceil(message.time / 100);
        let realIdx = TableInfo.realIdx[data.idx];
        let voiceCtrl = VoiceCtrl.getInstance();
        this.sprHorn.forEach((spr, i) => {
            spr.node.active = (i == realIdx);
        });
        if (db.getFloat(db.STORAGE_KEY.SET_SOUND, 1) > 0) {

            voiceCtrl.prepare(fileName);
            voiceCtrl.writeVoice(fileName, message.voice);
            voiceCtrl.play(fileName);
        }
        setTimeout(() => {
            voiceCtrl.release();
    ...
    changeBtnState() {
        this.btnExit.active = TableInfo.options.mode == 'CUSTOM' ? TableInfo.status == GameConfig.GameStatus.WAIT : TableInfo.status == GameConfig.GameStatus.DESTORY || TableInfo.status == GameConfig.GameStatus.WAIT;
    },
    /**同步手牌 */
    syncCard(data) {
        let currentCards = [];
