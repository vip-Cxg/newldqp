cc.Class({
    ...
    setData: function (data, handlers) {
        ...
        this.text('TodayRounds', this.data.todayRounds || this.data.rounds || 0);
        this.text('YesterdayRounds', this.data.yesterdayRounds || 0);
        this.text('TodayContribution', this.data.todayContribution || this.data.contribution || 0);
        this.text('YesterdayContribution', this.data.yesterdayContribution || 0);
        this.text('TodayResult', this.data.todayResult || this.data.result || 0);
        this.text('YesterdayResult', this.data.yesterdayResult || 0);
        this.text('Score', String(this.data.score || 0));
        this.text('TodayLabel', '今日：' + (this.data.todayResult || this.data.today || 0));
        this.text('YesterdayLabel', '昨日：' + (this.data.yesterdayResult || this.data.yesterday || 0));
        if (this.nodes.RoleBadge) this.nodes.RoleBadge.active = !!this.data.partner || this.data.role !== 'user';
        if (this.nodes.BtnSetPartner) this.nodes.BtnSetPartner.active = !this.data.partner && this.data.role === 'user';
