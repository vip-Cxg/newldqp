cc.Class({
    ...
    bindAll:function(){this.bind('BtnClose',this.close);this.bind('Mask',this.close);this.bind('BtnCancel',this.close);this.bind('BtnOK',function(){
        if(this.data.onOK){
            var ret=this.data.onOK();
            if(ret&&ret.then){
                ret.then(function(){this.close();}.bind(this)).catch(function(err){
                    console.error('[ConfirmPopup] submit failed', err);
                });
                return;
            }
        }
